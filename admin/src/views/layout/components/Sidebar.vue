...
export default class Sidebar extends Vue {
  ...
  setRouterData() {
    ...
    const secModuleMap = mapsRouterData.find((item: any) => {
      return String(item.id) === String(HomeModule.getModuleId);
    });
    if (!secModuleMap) {
      console.error("找不到对应模块，模块id:", HomeModule.getModuleId);
      return;
    }

    for (const it in secModuleMap.children) {
      // 遍历二级模块
        ...
        if (secModule.children.length > 1) {
          ...
          if (!isActive) {
            for (let i = 0; i < childrenData.length; ++i) {
              console.log(childrenData[i].id, this.$route.query.mode);
              if (childrenData[i]?.id === this.$route.query.mode) {
                active = true;
